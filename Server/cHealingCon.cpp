...
/**
 *	@file cHealingCon.cpp
 *	Implements functionality for healing containers.
 *
 *	This class is referenced whenever a healing container is created, used, or assessed.
...
/**
 *	Handles the message sent for the creation of healing containers in the world.
 *
 *	This function is called whenever a healing container should be created in the world for a client.
 *
 *	@return cMessage - Returns a Create Object (0x0000F745) server message.
 */
cMessage cHealingCon::CreatePacket()
{
	cMessage cmNull;
	return cmNull;
}
 ...
 *	@return cMessage - Returns a Create Object (0x0000F745) server message.
 */
cMessage cHealingCon::CreatePacketContainer( DWORD Container, DWORD ItemModelID )
{
	cMessage cmReturn;
...
//	cObject *pcObject = cWorldManager::FindObject( m_dwGUID );
	...
	cmReturn << 0x00020001L << 0x414L << 0x65L;
...
//	cmReturn << 0x34000000 + pcModel->m_dwUnknown_Blue;
	...
	cmReturn << 0x00284c18;		// Object Flags
 ...
 */
void cHealingCon::Action(cClient *who)
{
	cObject *pcObject = who->m_pcAvatar->FindInventory( m_dwGUID );
	...
	else
	{
		...
		who->AddPacket( WORLD_SERVER,aHealingKit,4);
 ...
 */
void cHealingCon::Assess( cClient *pcAssesser )
{
	cItemModels *pcModel = cItemModels::FindModel( m_dwItemModelID );
	cObject *pcObject = pcAssesser->m_pcAvatar->FindInventory( m_dwGUID );
