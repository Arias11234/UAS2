...
/**
 *	@file cScrolls.cpp
 *	Implements functionality for scrolls.
 *
 *	This class is referenced whenever a scrolls is created, used, or assessed.
...
/**
 *	Handles the message sent for the creation of scrolls in the world.
 *
 *	This function is called whenever a scroll should be created in the world for a client.
 *
 *	@return cMessage - Returns a Create Object (0x0000F745) server message.
 */
cMessage cScrolls::CreatePacket( )
{
	cMessage cmReturn;
 ...
 *	@return cMessage - Returns a Create Object (0x0000F745) server message.
 */
cMessage cScrolls::CreatePacketContainer(DWORD Container, DWORD ItemModelID)
{
	cMessage cmReturn;
	...
	cmReturn.pasteAlign(4);
	cmReturn << 0x00021081L << 0x414L << 0x65L;
 ...
 */
void cScrolls::Action(cClient *who)
{
	cItemModels *pcModel = cItemModels::FindModel( m_dwItemModelID );
	cObject *pcObject = cWorldManager::FindObject( m_dwGUID );
	
	cObject *pcObjInv = who->m_pcAvatar->FindInventory(m_dwGUID);

	if(pcObjInv)
	{
	cMessage cmReadScroll;
	cmReadScroll << 0xF7B0L << who->m_pcAvatar->GetGUID() << ++who->m_pcAvatar->m_dwF7B0Sequence << 0x004CL << 0x040BL << m_dwGUID << 0x8000L;
	who->AddPacket(WORLD_SERVER,cmReadScroll,4);
	
	cMessage cParticle = who->m_pcAvatar->Particle(28);
	cWorldManager::SendToAllInFocus( who->m_pcAvatar->m_Location, cParticle, 3 );
	cMessage cSound = who->m_pcAvatar->SoundEffect(28);
	...
	{
		cMasterServer::ServerMessage(ColorYellow,NULL,"Nope");
	}
}
 ...
 */
void cScrolls::Assess( cClient *pcAssesser )
{
	cItemModels *pcModel = cItemModels::FindModel( m_dwItemModelID );
